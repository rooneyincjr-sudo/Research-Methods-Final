...
write.csv(test_results,
          ...
          row.names = FALSE)
# ---- EXTRA (for interpreting Wilcoxon vs t-test): medians + IQR by group ----
conflict_summary <- analysis %>%
  group_by(military_group) %>%
  summarise(
    n = n(),
    median_conflict_years = median(conflict_years_post, na.rm = TRUE),
    iqr_conflict_years = IQR(conflict_years_post, na.rm = TRUE),
    mean_conflict_years = mean(conflict_years_post, na.rm = TRUE),
    sd_conflict_years = sd(conflict_years_post, na.rm = TRUE)
  ) %>%
  ungroup()

print(conflict_summary)
write.csv(conflict_summary,
          file = file.path("results", "tables", "table_06_h2_conflict_summary_median_iqr.csv"),
          row.names = FALSE)
